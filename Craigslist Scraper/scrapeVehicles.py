...
def runScraper():
    ...
    curs.execute('''CREATE TABLE IF NOT EXISTS vehicles(url STRING PRIMARY KEY, city STRING, price INTEGER, year INTEGER, make STRING,
    model STRING, condition STRING, cylinders STRING, fuel STRING, odometer INTEGER, title_status STRING, transmission STRING, VIN STRING,
    ...
    for city in citiesList:
        ...
        while not empty:
            ...
            for item in vehiclesList:
                ...
                townUrls.add(url)
                vehicleDict = {}
                vehicleDict["price"] = int(item[1].strip("$").replace(',', ''))
                ...
                if "make" in vehicleDict:
                    #make actually contains 3 variables that we'd like: year, make, and model (which we call make)
                    try:
                    ...
                    #if a match is found then we have our manufacturer, otherwise we set make to the entire string and leave the make blank
                    ...
                    model = model.strip()
                if ("cylinders" in vehicleDict) and vehicleDict["cylinders"] != 'other':
                    cylinders = int(vehicleDict["cylinders"][0])
                ...
                curs.execute('''INSERT INTO vehicles(url, city, price, year, make, model, condition, cylinders, fuel, odometer, title_status, transmission, VIN, drive, size, type,
                paint_color, image_url, lat, long)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''', (url, city, price, year, make, model, condition, cylinders, fuel, odometer, title_status, transmission, VIN, drive,
    ...
    db_df = pd.read_sql_query("SELECT * FROM vehicles", db)
    db_df.to_csv('vehicles_{}.csv'.format(today), index=False)
    print('Data has been exported to csv file')
